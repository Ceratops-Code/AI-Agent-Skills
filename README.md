...
| `skills/ceratops-credit-savings-analysis/scripts/credit-analysis-workflow.py` | Owns complete shared causal-episode evidence, protected final-state snapshots, finite shared Luna packets, one cross-surface Sol adjudication, explicit model execution, immutable persistence, lineage separation, resume state, batch contracts, ROI arithmetic, rendering, and scoped cleanup. |
