...
| `skills/ceratops-repo-lifecycle/scripts/promote-repository.py` | Prepares a clean local `release/*` checkout for fast-change, or fast-forwards selected committed task branches without creating or switching a checkout and optionally deploys the complete unpublished release delta from an existing exact-head checkout. |
