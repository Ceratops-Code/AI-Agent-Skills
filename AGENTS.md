...
- Before closing a skill-modifying task, do not stage or merge local skill-source changes into the active local `release/*` batch unless the user explicitly asked for staging, shipping, or local preview sync for that task. Exception: when the task's primary goal is creating a brand-new Ceratops skill through the `$ceratops-skill-lifecycle` create action, local staging into the active `release/*` batch is part of completion unless the user explicitly opts out. When staging is required, do it with the `$ceratops-skill-lifecycle` change-promotion action and verify the install there, or ship the active `release/*` batch through the `$ceratops-skill-lifecycle` ship-to-remote action. After shipping a batch, update the skills repo checkout from `origin/main`, verify installed skills are regenerated managed runtime copies, and report retained task worktrees or release branches; remove them only when cleanup was explicitly requested or is already part of the confirmed GitHub merge action.
