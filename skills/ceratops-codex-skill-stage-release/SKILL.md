...
- Skill-local merged-work cleanup and pending local work check: `scripts/check-pending-release-work.ps1 -CleanMerged` from the installed skill folder, or `skills/ceratops-codex-skill-stage-release/scripts/check-pending-release-work.ps1 -CleanMerged` from a source checkout, when that helper exists.
...
- Run `powershell -ExecutionPolicy Bypass -File .\\scripts\\install-skills.ps1 -Validate full` from the skill repo checkout when that installer and validation path exist.
...
- When the target repo lacks install or validation scripts, skip those steps and report that the repo does not provide them.
