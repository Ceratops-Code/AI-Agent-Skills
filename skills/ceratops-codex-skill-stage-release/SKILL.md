...
- If no installer exists, child-copy each affected source skill folder into its runtime target, then read back a changed sentinel file from each target before reporting it updated.
- If an affected runtime lives in an installed plugin cache, child-copy the source plugin folder into that cache target and read back changed sentinel files before reporting it updated.
