...
1. Require first-install authorization and an active AI-Agent-Skills source
   ...
   `C:\AI-Agents-Tools\ceratops_tool_manager`. It installs no Python or uv copy
   and changes no global Python libraries.
...
3. When registration is requested, register only this stdio service in the
   ...
   `-I -B`, the manager folder's `bin/ceratops_tool_manager.py` launcher, and
