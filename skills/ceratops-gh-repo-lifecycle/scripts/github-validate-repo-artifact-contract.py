...
* Artifact checks are audit-oriented. Local build/publish/consumer smoke checks
  are reported as NEEDS_REVIEW unless the contract can verify them from metadata.
...
from validator_levels import BLOCKING_LEVELS, count_by_level, has_blocking_findings, parse_levels
...
def default_contract_path(filename: str, *aliases: str) -> str:
    ...
    Source checkouts and installed skill copies carry these contracts as
    skill-local references. The CLI default probes the current directory and the
    owning lifecycle skill before falling back to the raw filename the caller
    supplied.
    """
    ...
    bases = (
        pathlib.Path.cwd(),
        pathlib.Path.cwd() / "skills" / "ceratops-gh-repo-lifecycle" / "references",
        repo_root / "skills" / "ceratops-gh-repo-lifecycle" / "references",
...
def parse_github_datetime(value: Any) -> dt.datetime | None:
    ...
    if not isinstance(value, str) or not value:
        return None
    raw = value.removesuffix("Z") + ("+00:00" if value.endswith("Z") else "")
    try:
...
def is_older_than(value: Any, days: int, *, now: dt.datetime | None = None) -> bool:
    """Return whether a GitHub timestamp is older than the policy threshold."""
...
def with_stale_reason(item: dict[str, Any], reason: str) -> dict[str, Any]:
    """Attach a compact policy reason without mutating fetched evidence."""
...
def name_matches_any(name: str, patterns: list[str]) -> bool:
    """Return whether a branch, tag, or release name matches any regex policy."""
...
def retention_days_finding(check_id: str, res: ApiResult, expected: dict[str, Any]) -> list[dict[str, Any]]:
    ...
    return [finding(check_id, "ERROR", "Actions artifact/log retention exceeds policy.", actual=days, expected=f"<= {max_days}", path="$.days")]
...
def stale_pull_request_candidates(open_prs: list[Any], expected: dict[str, Any]) -> list[dict[str, Any]]:
    """Return open PRs whose update age meets the stale review threshold."""

    threshold = int(expected.get("report_open_prs_older_than_days", 30))
    candidates: list[dict[str, Any]] = []
    for item in open_prs:
        if not isinstance(item, dict):
            continue
        if is_older_than(item.get("updated_at"), threshold):
            candidates.append(with_stale_reason(item, f"open PR updated more than {threshold} days ago"))
    return candidates
...
def stale_branch_candidates(
    ...
    """Return branch candidates that have no default, protection, PR, or retained-name signal."""

    ignored = set(expected.get("ignored_branch_names", []))
    retained_patterns = list(expected.get("retained_branch_name_patterns", []))
    open_pr_heads = {
        str((item.get("head") or {}).get("ref"))
        for item in open_prs
        if isinstance(item, dict) and isinstance(item.get("head"), dict) and (item.get("head") or {}).get("ref")
    }
    candidates: list[dict[str, Any]] = []
    for item in branches:
        ...
        if not name or name == default_branch or name in ignored:
            continue
        if item.get("protected") is True:
            continue
        if name in open_pr_heads:
            continue
        if name_matches_any(name, retained_patterns):
            continue
        candidates.append(with_stale_reason(item, "non-default unprotected branch without an open PR or retained-name policy"))
    return candidates


def stale_tag_candidates(tags: list[Any], releases: list[Any]) -> list[dict[str, Any]]:
    """Return tags that do not map to a visible GitHub release."""

    release_tags = {str(item.get("tag_name")) for item in releases if isinstance(item, dict) and item.get("tag_name")}
    candidates: list[dict[str, Any]] = []
    for item in tags:
        if not isinstance(item, dict):
            continue
        name = str(item.get("name") or "")
        if name and name not in release_tags:
            candidates.append(with_stale_reason(item, "tag has no matching GitHub release"))
    return candidates


def stale_release_candidates(releases: list[Any], tags: list[Any], expected: dict[str, Any]) -> list[dict[str, Any]]:
    """Return releases that violate deterministic stale-release policy."""

    draft_days = int(expected.get("draft_review_after_days", 7))
    prerelease_days = int(expected.get("prerelease_review_after_days", 30))
    required_asset_patterns = list(expected.get("required_asset_name_patterns", []))
    tag_names = {str(item.get("name")) for item in tags if isinstance(item, dict) and item.get("name")}
    candidates: list[dict[str, Any]] = []
    for item in releases:
        if not isinstance(item, dict):
            continue
        reasons: list[str] = []
        release_time = item.get("updated_at") or item.get("published_at") or item.get("created_at")
        if item.get("draft") is True and is_older_than(release_time, draft_days):
            reasons.append(f"draft release older than {draft_days} days")
        if item.get("prerelease") is True and is_older_than(release_time, prerelease_days):
            reasons.append(f"prerelease older than {prerelease_days} days")
        tag_name = str(item.get("tag_name") or "")
        if tag_names and tag_name and tag_name not in tag_names:
            reasons.append("release tag is not present in tag inventory")
        if required_asset_patterns:
            assets = [str(asset.get("name") or "") for asset in as_list(item.get("assets")) if isinstance(asset, dict)]
            missing = []
            for pattern in required_asset_patterns:
                try:
                    matched = any(re.search(pattern, asset) for asset in assets)
                except re.error:
                    matched = False
                if not matched:
                    missing.append(pattern)
            if missing:
                reasons.append(f"missing required release assets matching: {', '.join(missing)}")
        if reasons:
            candidates.append(with_stale_reason(item, "; ".join(reasons)))
    return candidates
...
def evaluate_repo_check(
    ...
    checks that require intent or quality judgment should return NEEDS_REVIEW or move
    ...
    owner_plan_obj = owner_info.get("plan")
    owner_plan = owner_plan_obj if isinstance(owner_plan_obj, dict) else {}
    owner_obj = repo_info.get("owner")
    owner = owner_obj if isinstance(owner_obj, dict) else {}
    ...
    rule_context = {
        ...
        "owner_is_org": owner.get("type") == "Organization",
    ...
    if check_id == "repo.identity_public_contract":
        ...
        if str(actual["owner"] or "").casefold() != str(params["owner"]).casefold():
            drifts.append(finding(check_id, "ERROR", "Owner mismatch.", actual["owner"], params["owner"], "$.owner"))
        if str(actual["name"] or "").casefold() != str(params["repo"]).casefold():
            drifts.append(finding(check_id, "ERROR", "Repo name mismatch.", actual["name"], params["repo"], "$.name"))
        if actual["default_branch"] != default_branch:
            drifts.append(finding(check_id, "ERROR", "Default branch mismatch.", actual["default_branch"], default_branch, "$.default_branch"))
        if actual["visibility"] not in expected_visibility:
            drifts.append(finding(check_id, "ERROR", "Unknown visibility.", actual["visibility"], sorted(expected_visibility), "$.visibility"))
        if actual["visibility"] == "public" and not actual["description"]:
            drifts.append(finding(check_id, "ERROR", "Public repo description is empty.", actual["description"], "non-empty", "$.description"))
    ...
    if check_id == "repo.topics_required":
        if repo_info.get("visibility") == "public" and not repo_info.get("archived") and not topics:
            return [finding(check_id, "ERROR", "Public active repo has no topics.", actual=[], expected="at least one topic")]
    ...
    if check_id in {"process.default_branch_enforcement_present", "process.default_branch_name_and_protection_indicator"}:
        ...
        return [finding(check_id, "ERROR", "Default branch has no readable protection or active branch ruleset.", actual={"protection": protection.message, "rulesets": rulesets.message}, expected="classic protection or active ruleset")]
    ...
    if check_id == "process.branch_protection_or_ruleset_posture":
        ...
        if not protection.ok or not isinstance(protection.data, dict):
            ...
            if active:
                return [finding(check_id, "NEEDS_REVIEW", "Active rulesets exist, but constraints need ruleset-detail review.", actual=active)]
            return [finding(check_id, "ERROR", "Branch protection posture is not verifiable.", actual=protection.message, expected="readable protection or ruleset detail")]
        p = protection.data
        drifts = []
        checks = p.get("required_status_checks") or {}
        if checks.get("strict") is not True:
            drifts.append(finding(check_id, "ERROR", "Strict status checks disabled.", checks.get("strict"), True, "$.required_status_checks.strict"))
        if not (checks.get("checks") or checks.get("contexts")):
            drifts.append(finding(check_id, "ERROR", "No required checks configured.", [], "one or more checks", "$.required_status_checks"))
        ...
        if reviews.get("dismiss_stale_reviews") is not True:
            drifts.append(finding(check_id, "ERROR", "Stale review dismissal disabled.", reviews.get("dismiss_stale_reviews"), True, "$.required_pull_request_reviews.dismiss_stale_reviews"))
        ...
        if (p.get("required_conversation_resolution") or {}).get("enabled") is not True:
            drifts.append(finding(check_id, "ERROR", "Conversation resolution not required.", None, True, "$.required_conversation_resolution"))
        if (p.get("enforce_admins") or {}).get("enabled") is not True:
            drifts.append(finding(check_id, "ERROR", "Admin enforcement disabled.", None, True, "$.enforce_admins"))
        for key in ("allow_force_pushes", "allow_deletions"):
            value = (p.get(key) or {}).get("enabled")
            if value is not False:
                drifts.append(finding(check_id, "ERROR", f"{key} should be disabled.", value, False, f"$.{key}"))
    ...
    if check_id == "process.maintainer_bypass_ruleset":
        ...
        drifts = []
        if missing:
            drifts.append(finding(check_id, "ERROR", "Expected maintainer bypass actor missing from rulesets.", actual=ruleset_evidence, expected=expected_actors))
    ...
    if check_id == "actions.permissions_policy":
        ...
        if data.get("enabled") is not True:
            drifts.append(finding(check_id, "ERROR", "Actions disabled.", data.get("enabled"), True, "$.enabled"))
    ...
    if check_id == "actions.workflow_token_permissions":
        ...
        if data.get("default_workflow_permissions") != "read":
            drifts.append(finding(check_id, "ERROR", "Default workflow token permission is not read.", data.get("default_workflow_permissions"), "read", "$.default_workflow_permissions"))
        if data.get("can_approve_pull_request_reviews") is not False:
            drifts.append(finding(check_id, "ERROR", "Workflow token can approve PR reviews.", data.get("can_approve_pull_request_reviews"), False, "$.can_approve_pull_request_reviews"))
        if workflows_with_permissions_write_all(local):
            drifts.append(finding(check_id, "ERROR", "Workflow files use permissions: write-all.", actual=workflows_with_permissions_write_all(local), expected="least privilege workflow permissions"))
        top_level_attestations = workflows_with_top_level_write_permission(local, "attestations")
        if top_level_attestations:
            drifts.append(finding(check_id, "ERROR", "Workflow-root attestations: write grants are not job scoped.", actual=top_level_attestations, expected="job-level attestations: write"))
        ...
        if top_level_artifact_metadata:
            drifts.append(finding(check_id, "ERROR", "Workflow-root artifact-metadata: write grants are not job scoped.", actual=top_level_artifact_metadata, expected="job-level artifact-metadata: write"))
    ...
    if check_id == "actions.workflow_sha_pinning":
        ...
        if unpinned:
            return [finding(check_id, "ERROR", "External workflow actions are not pinned to full SHAs.", actual=unpinned, expected="full 40-character SHA refs")]
        if not local.get("available"):
            return [finding(check_id, "NEEDS_REVIEW", "Local workflow files unavailable; cannot scan action refs.", actual=local.get("errors"))]
    ...
    if check_id == "actions.latest_default_branch_runs":
        ...
        if failures:
            return [finding(check_id, "ERROR", "Recent completed default-branch workflow run failed.", actual=[{"name": r.get("name"), "conclusion": r.get("conclusion"), "url": r.get("html_url")} for r in failures])]
        if not completed:
    ...
    if check_id == "security.security_and_analysis_status":
        ...
        if not repo_info.get("archived") and (security.get("dependency_graph") or {}).get("status") not in {"enabled", None}:
            drifts.append(finding(check_id, "ERROR", "Dependency graph is disabled.", actual=security.get("dependency_graph"), expected="enabled"))
        if repo_info.get("visibility") == "public":
            for key in ("secret_scanning", "secret_scanning_push_protection"):
                if (security.get(key) or {}).get("status") != "enabled":
                    drifts.append(finding(check_id, "ERROR", f"{key} is not enabled for a public repo.", actual=security.get(key), expected="enabled", path=f"$.{key}"))
    ...
    if check_id in {"security.vulnerability_alerts_enabled", "security.dependabot_security_updates_enabled"}:
        ...
        return [finding(check_id, "ERROR", "Endpoint is not enabled or could not be verified.", actual={"status": res.status, "message": res.message}, expected="enabled")]
    ...
    if check_id == "security.dependabot_config_file":
        ...
        if not dep_path:
            return [finding(check_id, "ERROR", "Package manifests exist but dependabot.yml is missing.", actual=ecosystems, expected=".github/dependabot.yml")]
        text = local.get("texts", {}).get(dep_path, "")
        if not text:
            return [finding(check_id, "NEEDS_REVIEW", "dependabot.yml exists but local contents are unavailable for deterministic schedule/ecosystem checks.", actual={"path": dep_path, "ecosystems": ecosystems})]
        drifts = []
        ...
        if "updates:" not in text:
            drifts.append(finding(check_id, "ERROR", "dependabot.yml has no updates section.", actual=dep_path, expected="updates:", path=dep_path))
        if "schedule:" not in text or "interval:" not in text:
            drifts.append(finding(check_id, "ERROR", "dependabot.yml updates entries need schedule intervals.", actual=dep_path, expected="schedule.interval", path=dep_path))
    ...
    if check_id in {"security.open_dependabot_alert_queue", "security.open_code_scanning_alert_queue"}:
        ...
        for item in alerts:
            ...
            if severity in {"critical", "high"}:
                high.append(item)
        if high:
            return [finding(check_id, "ERROR", "Open critical/high alerts exist.", actual=high, expected="0 critical/high alerts")]
    ...
    if check_id == "security.open_dependabot_pr_queue":
        ...
        if len(items) >= 1000:
            return [finding(check_id, "ERROR", "Dependabot PR search result limit was saturated.", actual=len(items), expected="< 1000")]
        if items:
    ...
    if check_id == "security.open_secret_scanning_alert_queue":
        alerts = open_alerts(result(fetched, check["endpoint"], params).data)
        if alerts:
            return [finding(check_id, "ERROR", "Open secret scanning alerts exist.", actual={"open_count": len(alerts)}, expected=0)]
    ...
    if check_id == "security.code_scanning_default_setup_or_custom":
        ...
        code_scanning_alerts = result(fetched, "/repos/${owner}/${repo}/code-scanning/alerts?state=open&per_page=100", params)
        if code_scanning_alerts.ok:
            return [finding(check_id, "PASS", "Code scanning appears available through custom setup.", actual={"default_setup": setup.data})]
        return [finding(check_id, "WARN", "Code scanning setup is not verifiable.", actual={"default_setup": setup.message, "alerts": code_scanning_alerts.message})]
    ...
    if check_id == "content.required_files_presence":
        ...
        if missing:
            return [finding(check_id, "ERROR", "Required contextual files are missing.", actual=missing, expected=required)]
    ...
    if check_id == "content.community_profile_public":
        ...
        if isinstance(health, int) and health < 70:
            drifts.append(finding(check_id, "ERROR", "Community profile health below contract minimum.", health, ">= 70", "$.health_percentage"))
        ...
        if repo_info.get("visibility") == "public" and owner_type == "Organization" and not repo_info.get("fork"):
            if profile.data.get("content_reports_enabled") is not True:
                drifts.append(finding(check_id, "ERROR", "Reported-content moderation is not enabled.", profile.data.get("content_reports_enabled"), True, "$.content_reports_enabled"))
    ...
    if check_id == "content.codeowners_valid":
        ...
        if errors:
            return [finding(check_id, "ERROR", "CODEOWNERS has errors.", actual=errors, expected=[])]
    ...
    if check_id == "content.dependencies_label_when_dependabot_uses_it":
        ...
        return [finding(check_id, "ERROR", "dependabot.yml references dependencies label but live label is missing.", actual=label.message, expected="label dependencies")]
    ...
    if check_id.startswith("stale_state."):
        ...
        if not res.ok:
            return [finding(check_id, "WARN", "Stale-state inventory could not be fetched.", actual={"status": res.status, "message": res.message})]
        expected = check.get("expected", {})
        inventory = as_list(res.data)
        candidates: list[dict[str, Any]] = []
        message = "No stale-state candidates found."
        candidate_message = "Stale-state candidates need review-owner classification before cleanup."

        if check_id == "stale_state.pull_requests":
            candidates = stale_pull_request_candidates(inventory, expected)
            message = "No stale open PR candidates found."
            candidate_message = "Open PRs older than the stale threshold need review-owner classification."
        ...
        if check_id == "stale_state.branches":
            ...
            open_prs_res = result(fetched, "/repos/${owner}/${repo}/pulls?state=open&per_page=100", params)
            if not open_prs_res.ok and inventory:
                return [finding(
                    check_id,
                    "WARN",
                    "Branch inventory fetched, but open PR inventory is unavailable for stale-branch classification.",
                    actual={"status": open_prs_res.status, "message": open_prs_res.message},
                    inventory=inventory,
                )]
            candidates = stale_branch_candidates(inventory, as_list(open_prs_res.data), default_branch, expected)
            message = "No stale branch candidates found."
            candidate_message = "Non-default unprotected branches without open PRs need review-owner classification."

        if check_id == "stale_state.tags":
            releases_res = result(fetched, "/repos/${owner}/${repo}/releases?per_page=100", params)
            if not releases_res.ok and inventory:
                return [finding(
                    check_id,
                    "WARN",
                    "Tag inventory fetched, but release inventory is unavailable for tag/release classification.",
                    actual={"status": releases_res.status, "message": releases_res.message},
                    inventory=inventory,
                )]
            candidates = stale_tag_candidates(inventory, as_list(releases_res.data))
            message = "No stale tag candidates found."
            candidate_message = "Tags without matching releases need review-owner classification."

        if check_id == "stale_state.releases":
            tags_res = result(fetched, "/repos/${owner}/${repo}/tags?per_page=100", params)
            if not tags_res.ok and inventory:
                return [finding(
                    check_id,
                    "WARN",
                    "Release inventory fetched, but tag inventory is unavailable for release classification.",
                    actual={"status": tags_res.status, "message": tags_res.message},
                    inventory=inventory,
                )]
            candidates = stale_release_candidates(inventory, as_list(tags_res.data), expected)
            message = "No stale release candidates found; published release history is retained."
            candidate_message = "Draft, prerelease, tag-mismatched, or asset-incomplete releases need review-owner classification."

        if candidates:
            return [finding(check_id, "NEEDS_REVIEW", candidate_message, actual=candidates, expected=expected, inventory=inventory)]
        return [finding(check_id, "PASS", message, actual=inventory, expected=expected, inventory=inventory)]
    ...
    return [finding(check_id, "NEEDS_REVIEW", f"Comparison {check.get('comparison')} is not automated by this checker yet.")]
...
def evaluate_artifact_check(check: dict[str, Any], params: dict[str, Any], local: dict[str, Any], types: dict[str, Any], registries: dict[str, Any], releases: Any) -> list[dict[str, Any]]:
    ...
    if check_id.startswith("common."):
        ...
        if check_id == "common.no_artifact_consistency":
            ...
            if "no_artifact" in artifacts and release_assets:
                return [finding(check_id, "ERROR", "No-artifact posture conflicts with uploaded release assets.", actual=release_assets)]
    ...
    if check_id.startswith("pypi."):
        ...
        if check_id == "pypi.metadata_and_build":
            ...
            if not text:
                return [finding(check_id, "ERROR", "pyproject.toml is missing or unreadable.", expected="pyproject.toml")]
            ...
            drifts = [finding(check_id, "ERROR", "PyPI project metadata is incomplete.", actual=missing, path="pyproject.toml")] if missing else []
        ...
        if check_id == "pypi.trusted_publishing_and_attestations":
            ...
            if "id-token: write" not in text:
                drifts.append(finding(check_id, "ERROR", "PyPI Trusted Publishing workflow lacks id-token: write.", expected="job-scoped id-token: write"))
        ...
        if check_id == "pypi.live_version_install_smoke":
            ...
            return [finding(check_id, "ERROR", "PyPI live metadata lookup failed.", actual=bad)] if bad else [finding(check_id, "PASS", "PyPI project and latest version metadata resolved.", actual=registries["pypi"])]
    ...
    if check_id.startswith("npm."):
        ...
        if check_id == "npm.metadata_and_package_contents":
            try:
                ...
                return [finding(check_id, "ERROR", "package.json is invalid JSON.", actual=str(exc), path="package.json")]
            ...
            drifts = [finding(check_id, "ERROR", "npm package metadata is incomplete.", actual=missing, path="package.json")] if missing else []
        ...
        if check_id == "npm.trusted_publishing_and_provenance":
            ...
            if bad_node_versions:
                drifts.append(finding(check_id, "ERROR", "npm trusted publishing workflow uses a Node version below the documented minimum.", actual=bad_node_versions, expected=">= 22.14.0"))
            ...
            if bad_npm_versions:
                drifts.append(finding(check_id, "ERROR", "npm trusted publishing workflow uses an npm CLI version below the documented minimum.", actual=bad_npm_versions, expected=">= 11.5.1"))
            ...
            if github_publish_text and "id-token: write" not in github_publish_text:
                drifts.append(finding(check_id, "ERROR", "npm trusted publishing/provenance workflow lacks id-token: write.", expected="job-scoped id-token: write"))
            if github_publish_text and re.search(r"(?im)runs-on:\s*(?:\[.*)?self-hosted\b", github_publish_text):
                drifts.append(finding(check_id, "ERROR", "npm trusted publishing does not support self-hosted GitHub runners.", expected="GitHub-hosted runner or scoped token fallback"))
            ...
            if re.search(r"(?i)trusted publishing", text) and not re.search(r"(?i)(allow(?:ed)?[- ]actions?|allow[- ]publish|allow[- ]stage[- ]publish|npm\s+trust)", text):
                drifts.append(finding(check_id, "NEEDS_REVIEW", "npm trusted publisher allowed actions are not visible in workflow text.", expected="allowed npm publish or npm stage publish action matches release policy"))
        ...
        if check_id == "npm.live_package_version_smoke":
            ...
            return [finding(check_id, "ERROR", "npm live metadata lookup failed.", actual=bad)] if bad else [finding(check_id, "PASS", "npm package and latest dist-tag metadata resolved.", actual=registries["npm"])]
    ...
    if check_id.startswith("docker_oci."):
        ...
        if check_id == "docker_oci.dockerfile_and_context":
            ...
            if not dockerfiles:
                return [finding(check_id, "ERROR", "No Dockerfile found for Docker/OCI artifact.", expected="Dockerfile")]
            drifts = []
            if ".dockerignore" not in local.get("files", []):
                drifts.append(finding(check_id, "ERROR", ".dockerignore is missing.", expected=".dockerignore"))
            for path in dockerfiles:
                text = local.get("texts", {}).get(path, "")
                if not text:
                    drifts.append(finding(check_id, "NEEDS_REVIEW", "Dockerfile content unavailable for deterministic inspection.", actual=path, path=path))
                    continue
                if not re.search(r"(?im)^FROM\s+\S+", text):
                    drifts.append(finding(check_id, "ERROR", "Dockerfile has no FROM instruction.", actual=path, path=path))
                ...
                if SECRET_NAME_RE.search(text):
                    drifts.append(finding(check_id, "ERROR", "Dockerfile declares secret-like ARG or ENV names.", expected="secret mount or CI secret", path=path))
        ...
        if check_id == "docker_oci.build_publish_and_pull":
            if registries.get("dockerhub"):
                ...
                return [finding(check_id, "ERROR", "Docker Hub repository or latest tag metadata lookup failed.", actual=bad)] if bad else [finding(check_id, "PASS", "Docker Hub repository and latest tag metadata resolved.", actual=registries["dockerhub"])]
            return [finding(check_id, "NEEDS_REVIEW", "No Docker Hub image name was provided; GHCR or other registry needs registry-specific verification.")]
        if check_id == "docker_oci.provenance_and_sbom":
            ...
            if any(step.get("default_provenance_applies") for step in default_provenance_steps) and secret_like_dockerfiles:
                drifts.append(finding(check_id, "ERROR", "Default Docker max provenance can expose secret-like build args or env names.", actual=secret_like_dockerfiles, expected="secret mounts or CI secrets"))
    ...
    return [finding(check_id, "NEEDS_REVIEW", "Artifact-specific check requires registry or build-system review.", actual={"artifact_surface": sorted(artifacts)})]
...
STALE_INVENTORY_KEYS = {
    "stale_state.pull_requests": "pull_requests",
    "stale_state.branches": "branches",
    "stale_state.tags": "tags",
    "stale_state.releases": "releases",
    "stale_state.local_path_references": "local_path_references",
}
...
def summarize(findings: list[dict[str, Any]]) -> dict[str, int]:
    ...
    return count_by_level(findings)


def compact_value(value: Any, *, sample_limit: int = 5) -> Any:
    """Return a bounded representation suitable for summary output."""

    if isinstance(value, list):
        return {"count": len(value), "sample": value[:sample_limit]}
    if isinstance(value, dict):
        encoded = canonical(value)
        if len(encoded) <= 1000:
            return value
        keys = sorted(str(key) for key in value.keys())
        return {"keys": keys, "sample": {key: value[key] for key in keys[:sample_limit] if key in value}}
    return value


def compact_stale_item(kind: str, item: Any) -> Any:
    """Return stable stale-state sample fields for one inventory item."""
    ...
    if kind == "pull_requests":
        ...
        if reason:
            compact["stale_reason"] = reason
        return compact
    if kind == "branches":
        commit = item.get("commit")
        compact = {"name": item.get("name"), "protected": item.get("protected"), "sha": commit.get("sha") if isinstance(commit, dict) else None}
        if reason:
            compact["stale_reason"] = reason
        return compact
    if kind == "tags":
        commit = item.get("commit")
        compact = {"name": item.get("name"), "sha": commit.get("sha") if isinstance(commit, dict) else None, "url": item.get("zipball_url") or item.get("tarball_url")}
        if reason:
            compact["stale_reason"] = reason
        return compact
    if kind == "releases":
        ...
        if reason:
            compact["stale_reason"] = reason
        return compact
    if kind == "local_path_references":
        return {"path": item.get("path"), "pattern": item.get("pattern")}
    return compact_value(item)


def stale_state_inventory(findings: list[dict[str, Any]], *, sample_limit: int = 5) -> dict[str, Any]:
    """Extract compact stale-state inventories from full check evidence."""

    inventory: dict[str, Any] = {}
    for item in findings:
        kind = STALE_INVENTORY_KEYS.get(str(item.get("check_id")))
        if not kind:
            continue
        actual = item.get("inventory", item.get("actual"))
        if not isinstance(actual, list):
            continue
        inventory[kind] = {
            "count": len(actual),
            "sample": [compact_stale_item(kind, entry) for entry in actual[:sample_limit]],
        }
    return inventory


def compact_finding(item: dict[str, Any], *, sample_limit: int = 5) -> dict[str, Any]:
    """Return a bounded finding record for human and summary outputs."""

    stale_kind = STALE_INVENTORY_KEYS.get(str(item.get("check_id")))
    compact = {
    ...
    if "actual" in item:
        actual = item["actual"]
        if stale_kind and isinstance(actual, list):
            compact["actual"] = {
                "count": len(actual),
                "sample": [compact_stale_item(stale_kind, entry) for entry in actual[:sample_limit]],
            }
        else:
            compact["actual"] = compact_value(actual, sample_limit=sample_limit)
    if "expected" in item:
        compact["expected"] = compact_value(item["expected"], sample_limit=sample_limit)
    return compact


def build_summary_report(report: dict[str, Any], levels: list[str], *, sample_limit: int = 5) -> dict[str, Any]:
    """Build the compact report used by text and `--summary-json` output."""

    selected = [item for item in report["findings"] if item.get("level") in set(levels)]
    approved = [item for item in report["approved_drift"] if item.get("level") in set(levels)]
    return {
        "repo": report["repo"],
        ...
        "selected_counts": {
            "github": report["github_check_count"],
            "code": report["code_check_count"],
            "artifact": report["artifact_check_count"],
        },
        "fetched_endpoints": report["fetched_endpoints"],
        "types": report["types"],
        "findings": [compact_finding(item, sample_limit=sample_limit) for item in selected],
        "approved_drift": [compact_finding(item, sample_limit=sample_limit) for item in approved],
        "stale_state_inventory": stale_state_inventory(report["findings"], sample_limit=sample_limit),
        "local_scan": report["local_scan"],
    }
...
def print_human(report: dict[str, Any], levels: list[str]) -> None:
    ...
    if summary["stale_state_inventory"]:
        print("Stale-state inventory:")
        for kind, data in summary["stale_state_inventory"].items():
            print(f"  {kind}: count={data['count']} sample={json.dumps(data['sample'], sort_keys=True)}")
...
def main() -> int:
    ...
    output_group.add_argument("--json", action="store_true", help="Print the complete machine-readable JSON report.")
    output_group.add_argument("--summary-json", action="store_true", help="Print compact machine-readable summary JSON.")
    parser.add_argument("--levels", help="Comma-separated levels included in text or summary output. Defaults to ERROR,WARN,NEEDS_REVIEW.")
    ...
    if args.json and args.levels:
        parser.error("--levels applies to text or --summary-json output, not full --json")
    try:
        output_levels = parse_levels(args.levels)
    ...
    if args.apply:
        drift_ids = {item["check_id"] for item in findings if item["level"] in BLOCKING_LEVELS}
        ...
        if applied_success_ids:
            findings = [
                item
                for item in findings
                if not (item["level"] in BLOCKING_LEVELS and item["check_id"] in applied_success_ids)
            ]
    ...
    if args.json:
        ...
        print(json.dumps(build_summary_report(report, output_levels), indent=2, sort_keys=True))
    else:
        print_human(report, output_levels)
    return 2 if has_blocking_findings(findings) and not args.no_fail else 0
