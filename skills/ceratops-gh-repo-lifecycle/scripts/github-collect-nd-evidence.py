...
review owner. It collects evidence once per surface instead of asking the
review owner to run many separate commands.
"""
...
def compact_api(result: Any) -> dict[str, Any]:
...
def org_evidence(args: argparse.Namespace) -> dict[str, Any]:
    ...
    return {
        ...
        "evidence_command": f"python skills/ceratops-gh-repo-lifecycle/scripts/github-collect-nd-evidence.py --surface org --org {params['org_login']} --json",
...
def repo_or_artifact_evidence(args: argparse.Namespace, surface: str) -> dict[str, Any]:
    ...
    code_nd_checks = {
        ...
        "ND.code.comment-sufficiency": ["skills/ceratops-gh-repo-lifecycle/references/code-comment-nondeterministic-contract.md", "scripts", "README"],
        "ND.code.sources-current-doc-recheck": ["contract-source-docs.json"],
    }
    if surface == "repo":
        nd_checks = github_nd_checks
        evidence_command = f"python skills/ceratops-gh-repo-lifecycle/scripts/github-collect-nd-evidence.py --surface repo --repo {params['owner']}/{params['repo']} --local-repo-path <PATH> --json"
    elif surface == "code":
        nd_checks = code_nd_checks
        evidence_command = f"python skills/ceratops-gh-repo-lifecycle/scripts/github-collect-nd-evidence.py --surface code --repo {params['owner']}/{params['repo']} --local-repo-path <PATH> --json"
    else:
        nd_checks = {
        ...
        evidence_command = f"python skills/ceratops-gh-repo-lifecycle/scripts/github-collect-nd-evidence.py --surface artifact --repo {params['owner']}/{params['repo']} --local-repo-path <PATH> --json"
    common.update(
        {
            "surface": surface,
