...
- (D) Merged-work cleanup and pending local work check:
  ...
  -SkillsRepoRoot <repo> -CleanMergedBranches` from a source checkout, or
  `scripts/check-pending-release-work.ps1 -CleanMergedBranches` from the
  installed skill folder.
...
- (D) Run `stage-skill-release-branch.ps1` with only branches that passed the
  ...
  merge-base HEAD BRANCH) BRANCH`, branch merges, runtime installation,
  validation, merged-work cleanup, pending-work detection, and compact
...
- (D) Treat the staged release helper's successful `validation` and `install`
  fields as the validation and install result when it ran the skill-lifecycle
  validator and runtime installer; otherwise run those skill-lifecycle scripts
  directly from the skill repo checkout when they exist.
