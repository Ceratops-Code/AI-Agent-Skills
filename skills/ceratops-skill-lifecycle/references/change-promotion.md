...
Promote committed skill branches into the skill repo checkout's local release
branch so Codex can use one coherent unpublished repo snapshot. In this repo,
update installed runtime only when this action is intentionally selected. In
another skill repo, merge and run only the install, validation, cleanup, or
runtime steps that repo provides.
...
- (D) Staged release helper:
  `skills/ceratops-skill-lifecycle/scripts/stage-skill-release-branch.ps1
  -SkillsRepoRoot <repo> -ApprovedBranch BRANCH [-ApprovedBranch BRANCH...]
  -MainBranch main -ReleaseBranch release/local -RemoteName origin` from a
  source checkout, or `scripts/stage-skill-release-branch.ps1` from the
  installed skill folder.
- (D) Release branch preparation:
  `skills/ceratops-skill-lifecycle/scripts/prepare-release-branch.ps1
  -SkillsRepoRoot <repo> -MainBranch main -ReleaseBranch release/local
  -RemoteName origin` from a source checkout, or
  `scripts/prepare-release-branch.ps1` from the installed skill folder.
- (D) Merged-work cleanup and pending local work check:
  `skills/ceratops-skill-lifecycle/scripts/check-pending-release-work.ps1
  -SkillsRepoRoot <repo> -CleanMerged` from a source checkout, or
  `scripts/check-pending-release-work.ps1 -CleanMerged` from the installed skill
  folder.
...
- Which install, validation, cleanup, and runtime update helpers are absent in a
  non-Ceratops repo and should be skipped.
...
- (D) Run `stage-skill-release-branch.ps1` with only branches that passed the
  ...
  merge-base HEAD BRANCH) BRANCH`, branch merges, installer validation,
  merged-work cleanup, pending-work detection, and compact ready/not-ready
  output.
- If the staged release helper is unavailable in a compatible non-Ceratops skill
  repo, use the lower-level preparation and pending-work helpers plus the same
  deterministic git, install, and cleanup checks.
- If the skills repo checkout is dirty before staging, stop instead of merging
  into it blindly.

#### 3. Install and validate staged state

- (D) Treat the staged release helper's successful `validation` field as the
  install/validation result when it ran `scripts/install-skills.ps1 -Validate
  full`; otherwise run `powershell -ExecutionPolicy Bypass -File
  .\scripts\install-skills.ps1 -Validate full` from the skill repo checkout when
  installer and validation path exist.
- Confirm each installed Ceratops skill copy has
  `.ceratops-runtime-manifest.json` and was regenerated by the installer when
...
- Run the skill-local pending local work check before reporting the batch as
  ready to publish. Use `-CleanMerged` for post-merge cleanup; omit it for final
  read-only pending-work pass unless cleanup was explicitly requested.
