...
# Install or refresh managed runtime skill copies from a compatible source repo.
#
...
# Fail before any install write when the target repository or selected helper
...
$targetedInstall = $null -ne $Skill -and $Skill.Count -gt 0
$buildSkillNames = @()
if ($targetedInstall) {
...
if (-not $targetedInstall) {
    # Full installs remove stale folders owned by this source repository.
    # Any explicit -Skill request is targeted and never removes stale folders.
